...
class Bot:
    ...
    def log_write(self, scenario, action, log_time, user='', log_user_id=''):
    ...
    # -------------------------------------------------------------------------
    def recognize_update(self, update_object):
    ...
    def c_help(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
        return u'Справка по командам:'

    def c_start_game(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
    ...
    def c_end_game(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
    ...
    def c_change_article(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
        if not session_continues or not storage.data[user_id]['state'] == 'game':
    ...
    def c_hitler_mode(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
        if not session_continues or not storage.data[user_id]['state'] == 'game':
    ...
    def c_score(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
        return 'get_score()'

    def c_open(self, session_continues, storage, user_id, username):
        if not session_continues:
            storage.new_user(username, user_id)
        if not session_continues or not storage.data[user_id]['state'] == 'game':
